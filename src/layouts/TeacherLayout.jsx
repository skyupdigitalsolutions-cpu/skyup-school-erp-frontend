...
const NAV_ITEMS = [
  ...
  // Add teacher-specific modules here as they're built (attendance,
  // homework, timetable, class-teacher tools, ...).
];
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-white/10 ${collapsed ? 'justify-center' : ''}`}>
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shrink-0 shadow-lg text-xs font-bold">
          {initial}
        </div>
        {!collapsed && (
          <div>
            <p className="text-sm font-bold leading-tight tracking-wide">School ERP</p>
            <p className="text-[10px] text-white/40 font-medium uppercase tracking-widest">Teacher</p>
          </div>
        )}
      </div>
  ...
  return (
    <header className="h-14 bg-white border-b flex items-center gap-4 px-5 sticky top-0 z-30 shadow-sm">
      <div className="flex-1">
        <h2 className="text-sm font-semibold text-gray-800">Teacher Portal</h2>
      </div>
